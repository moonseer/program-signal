...
MCP security guidance forbids **token passthrough**: a server must not accept tokens that were not issued **for that MCP server** and forward them to a downstream API. Passthrough breaks audience checks, audit, and the trust boundary between client, server, and API.
