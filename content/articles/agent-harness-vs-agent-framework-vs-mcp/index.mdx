...
<Callout type="watch">
MCP is an open protocol for connecting LLM applications to external data sources and tools.<Cite sourceId="SRC-MCP-001" /> That sentence is from the specification. It is not a runtime. Stretching it into an architecture leaves blast radius to whoever happened to start the server.
</Callout>
