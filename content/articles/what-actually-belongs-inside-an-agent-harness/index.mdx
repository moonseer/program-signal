...
**Policy at the point of action.** Not a paragraph in a system prompt. An allow-list, an identity, a budget, a refusal. MCP tells implementors that tools are arbitrary code execution and that hosts must obtain consent before invoking them. Mapping that duty onto a named layer is the harness decision.
...
**Cluster scheduling.** Kubernetes schedules Pods. That is infrastructure. The harness may request a Pod. It should not become a second scheduler.
