...
**Identity.** Who the run is, in infrastructure terms. A Kubernetes service account can name a process. The harness still has to bind that name to a tool allow-list.
...
MCP standardizes how a host talks to a server that exposes tools, resources, and prompts. It does not schedule the server, issue its credentials, or decide what happens when the server hangs. The specification is explicit that security principles are not enforced at the protocol level.
